...
const updateKeyObject = async () => {
  try {
    const keyPrice = await Methods.getKeyPriceFromBackpackTF();
    keyobj = keyPrice;
    ...
    const keyItem = {
      ...
      buy: {
        keys: 0,
        metal: keyPrice.metal - 0.11,
      },
      sell: {
        keys: 0,
        metal: keyPrice.metal,
      },
    };

    Methods.addToPricelist(keyItem, PRICELIST_PATH);
    socketIO.emit("price", keyItem);
  } catch (err) {
